...
chart_minor=$(get_minor_version "v$CURRENT_VERSION")
...
LATEST_CHART_VERSION=$CURRENT_APP_VERSION
...
if [[ "$CHART_VERSION_OVERRIDDEN" == "true" ]]; then
    ...
    debug "Using override chart version value $CHART_VERSION_OVERRIDDEN"
...
elif [[ "$GITHUB_EVENT_NAME" == "workflow_dispatch" ]] || ( [[ "$GITHUB_EVENT_NAME" == "schedule" ]] && [[ "$APP_VERSION" != "$LATEST_APP_VERSION" ]] ); then
