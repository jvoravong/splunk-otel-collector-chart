...
# Check and set CHART_VERSION. Use the default from Chart.yaml if the environment variable is unset or set to an empty string.
if [[ -z "${CHART_VERSION}" ]]; then
    CHART_VERSION=$(grep "^version:" "${CHART_FILE_PATH}" | awk '{print $2}')
fi
# Check and set APP_VERSION. Use the default from Chart.yaml if the environment variable is unset or set to an empty string.
if [[ -z "${APP_VERSION}" ]]; then
    APP_VERSION=$(yq e ".appVersion" "${CHART_FILE_PATH}")
fi
# Set CREATE_BRANCH to true if it is unset or set to an empty string.
if [[ -z "${CREATE_BRANCH}" ]]; then
    CREATE_BRANCH=true
else
    # Ensure CREATE_BRANCH is treated as a boolean.
    CREATE_BRANCH=$( [[ "${CREATE_BRANCH}" == true ]] && echo true || echo false )
fi
