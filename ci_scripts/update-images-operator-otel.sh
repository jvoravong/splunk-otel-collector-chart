...
# Create a temporary file to store docker image repository location information
setd "TEMP_MAIN_FILE" "$SCRIPT_DIR/temp_main.out"
setd "TEMP_IMAGE_REPOS" "$SCRIPT_DIR/temp_image_repos.out"
...
# This gives us a list of image repositories.
    ...
    if [[ "$IMAGE_KEY" =~ ^autoinstrumentation-.* ]]; then
        ...
        if [[ -z "${REPOSITORY_LOCAL}" || "${REPOSITORY_LOCAL}" != *"splunk"* ]]; then
          ...
          if [[ -z "${TAG_LOCAL}" || "${TAG_LOCAL}" == "null" || "${TAG_LOCAL}" != "$TAG_UPSTREAM" ]]; then
            ...
            # Skopeo validates the existence of the Docker images no matter the current host architecture
